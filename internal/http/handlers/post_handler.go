...
type postService interface {
	...
	Update(ctx context.Context, post *post.Post, updatePostRequest requests.UpdatePostRequest) error
...
func (p *PostHandlers) CreatePost(c echo.Context) error {
	...
	if err != nil {
		return responses2.ErrorResponse(c, http.StatusUnauthorized, "Unauthorized")
	}

	var createPostRequest requests.CreatePostRequest
	if err := c.Bind(&createPostRequest); err != nil {
		return responses2.ErrorResponse(c, http.StatusBadRequest, "Failed to bind request: "+err.Error())
	}

	if err := createPostRequest.Validate(); err != nil {
		return responses2.ErrorResponse(c, http.StatusBadRequest, "Required fields are empty")
	}
	...
	if err := p.postService.Create(c.Request().Context(), post); err != nil {
		return responses2.ErrorResponse(c, http.StatusBadRequest, "Failed to create post: "+err.Error())
	}

	return responses2.MessageResponse(c, http.StatusCreated, "Post successfully created")
}

func (p *PostHandlers) GetPosts(c echo.Context) error {
	posts, err := p.postService.GetPosts(c.Request().Context())
	if err != nil {
		return responses2.ErrorResponse(c, http.StatusNotFound, "Failed to get all posts: "+err.Error())
	}

	response := responses2.NewPostResponse(posts)
	return responses2.Response(c, http.StatusOK, response)
}
...
func (p *PostHandlers) UpdatePost(c echo.Context) error {
	...
	if err != nil {
		return responses2.ErrorResponse(c, http.StatusBadRequest, "Failed to parse post id: "+err.Error())
	}
	...
	if errors.Is(err, errors2.ErrPostNotFound) {
		return responses2.ErrorResponse(c, http.StatusNotFound, "Post not found")
	} else if err != nil {
		return responses2.ErrorResponse(c, http.StatusInternalServerError, "Failed to find post: "+err.Error())
	}

	if post.UserID != auth.ID {
		return responses2.ErrorResponse(c, http.StatusForbidden, "Forbidden")
	}

	if err := p.postService.Update(c.Request().Context(), &post, updatePostRequest); err != nil {
		return responses2.ErrorResponse(c, http.StatusInternalServerError, "Failed to update post: "+err.Error())
	}

	return responses2.MessageResponse(c, http.StatusOK, "Post successfully updated")
}
...
func (p *PostHandlers) DeletePost(c echo.Context) error {
	...
	if err := p.postService.Delete(c.Request().Context(), &post); err != nil {
		return responses2.ErrorResponse(c, http.StatusInternalServerError, "Failed to delete post: "+err.Error())
	}

	return responses2.MessageResponse(c, http.StatusNoContent, "Post deleted successfully")
}
